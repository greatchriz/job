...
Route::get('/job-titles', function () {
   ...
   // convert all the company logos to an array
    ...
    foreach ($jobTitleElements as $jobTitleElement) {
        $jobTitles[] = $jobTitleElement->textContent;
    }
    ...
    // lets say foreach job title i want to echo the job title, and a job description  and a company logo that has the same key as the job title

    foreach ($fileContents['jobTitle'] as $jobTitle) {
        echo $jobTitle . '<br>';
        foreach ($fileContents['jobDescription'] as $jobDescription) {
            echo $jobDescription . '<br>';
        }
        foreach ($fileContents['companyLogo'] as $companyLogo) {
            echo $companyLogo . '<br>';
        }
    }
